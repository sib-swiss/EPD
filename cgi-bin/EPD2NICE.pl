...
#    {
	...
	open(SEQ, "fetch -c ../etc/fetch.conf -w 101 epdseq:$AC\[9950..10050\] |"); #or print R1 "cannot open epdseq fetch: $!\n";
	while(<SEQ>){
	    ($seq)=(/([ACGTN]{101})/);
	}
	close(SEQ);
	...
	open($RFILE, '>', "$WWWTmpDir/epd_nice$job.R");
	print $RFILE "bitmap(\"$WWWTmpDir/epd_nice$job.png\",type=\"png16\",width=9,height=6,res=80);
	    ...
	    dev.off();";
	close($RFILE);
	system(`R --no-save --silent < $WWWTmpDir/epd_nice$job.R`);
