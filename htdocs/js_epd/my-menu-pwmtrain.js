...
function getSeqLibMenu(sel)
{
	var seq_db = sel.options[sel.selectedIndex].value;
	document.getElementById('seqsample').options.length = 0;	// Empty Sample select box
	if(seq_db.length>0){
		var index = ajax.length;
		ajax[index] = new sack();

		ajax[index].requestFile = 'getSeqLib.php?seq_db='+seq_db;	// Specifying which file to get
		ajax[index].onCompletion = function(){ createSeqLib(index) };	// Specify function that will be executed after file has been found
		ajax[index].runAJAX();		// Execute AJAX function
	}
}

function createSeqLib(index)
{
	var obj = document.getElementById('seqsample');
	eval(ajax[index].response);	// Executing the response from Ajax as Javascript code
        var seq_name = obj.options[0].value;
}
